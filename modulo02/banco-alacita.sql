...
$$
    ...
    BEGIN
        ...
        SELECT id,
               'CTA-' || substring(id::text from 1 for 8) || '-' || trunc(random() * 1000),
               CASE (random() * 2)::int WHEN 0 THEN 'BOB' WHEN 1 THEN 'USD' ELSE 'USDT' END,
               1000000.00
        FROM clientes;
         COMMIT;
        ...
        FOR i IN 1..3000000
            LOOP
                ...
                VALUES (r_origen, r_destino, 'TRANSFERENCIA', v_monto, 'BOB')
                RETURNING id INTO v_trans_id;

                UPDATE cuentas SET saldo = saldo - v_monto WHERE id = r_origen RETURNING saldo INTO v_saldo_origen;
                INSERT INTO movimientos (cuenta_id, transferencia_id, tipo_movimiento, monto, saldo_posterior,
                                         descripcion)
        ...
        FOR i IN 1..1000000
            LOOP
                ...
                INSERT INTO prestamos (cliente_id, cuenta_desembolso_id, monto_principal, tasa_interes_anual,
                                       plazo_meses, fecha_desembolso)
                VALUES (r_cliente_id, r_origen, v_monto, 12.00, 12, v_fecha_desembolso)
                RETURNING id INTO v_prestamo_id;

                v_cuota := v_monto * (v_tasa_mensual * power(1 + v_tasa_mensual, 12)) /
                           (power(1 + v_tasa_mensual, 12) - 1);
                ...
                FOR mes IN 1..12
                    LOOP
                        ...
                        IF mes = 12 THEN
                            v_capital_mes := v_saldo_restante; v_cuota := v_capital_mes + v_interes_mes;
                        END IF;
                        v_saldo_restante := v_saldo_restante - v_capital_mes;
                        v_estado_pago :=
                                CASE WHEN v_fecha_vencimiento < CURRENT_DATE THEN 'PAGADO' ELSE 'PENDIENTE' END;

                        INSERT INTO prestamos_plan_pagos (prestamo_id, numero_cuota, fecha_vencimiento, monto_cuota,
                                                          componente_capital, componente_interes, estado_pago)
                        VALUES (v_prestamo_id, mes, v_fecha_vencimiento, v_cuota, v_capital_mes, v_interes_mes,
                                v_estado_pago);
                    END LOOP;
            END LOOP;
...
BEGIN
    ...
    INTO v_cuenta_origen_id, v_saldo_actual_origen, v_moneda_origen
    FROM cuentas
    ...
    FOREACH v_numero_cuenta_dest IN ARRAY p_numeros_cuenta_destino
        LOOP

            -- Buscar la cuenta del empleado
            SELECT id, moneda
            INTO v_cuenta_dest_id, v_moneda_dest
            FROM cuentas
            ...
            VALUES (v_cuenta_origen_id, v_cuenta_dest_id, 'PAGO_PLANILLA', p_monto_por_empleado, v_moneda_origen,
                    'COMPLETADA')
            RETURNING id INTO v_trans_id;

            -- Debitar a la empresa
            UPDATE cuentas
            SET saldo = saldo - p_monto_por_empleado
            ...
            VALUES (v_cuenta_origen_id, v_trans_id, 'DEBITO', p_monto_por_empleado, v_saldo_origen_post,
                    'Pago Planilla a: ' || v_numero_cuenta_dest);
            ...
            INSERT INTO movimientos (cuenta_id, transferencia_id, tipo_movimiento, monto, saldo_posterior, descripcion)
            VALUES (v_cuenta_dest_id, v_trans_id, 'CREDITO', p_monto_por_empleado, v_saldo_destino_post,
                    'Abono Planilla de: ' || p_numero_cuenta_origen);
...
$$
    ...
    BEGIN
        ...
        SELECT array_agg(numero_cuenta)
        INTO v_empleados_array
        FROM (SELECT numero_cuenta
              FROM cuentas
              WHERE moneda = 'BOB'
                AND numero_cuenta != v_cuenta_empresa
              LIMIT 3) AS subquery;
...
SELECT t.id,
       t.fecha,
       cco.nombres      as cliente_origen,
       co.numero_cuenta as cuenta_origen,
       ccd.nombres      as cliente_destino,
...
FROM transferencias t
         ...
         INNER JOIN clientes ccd on cd.cliente_id = ccd.id
         INNER JOIN cuentas co on t.cuenta_origen_id = co.id
         INNER JOIN clientes cco on co.cliente_id = cco.id;


INSERT INTO cuentas (numero_cuenta, moneda, estado, saldo) VALUES
('BCO-CAPITAL-BOB', 'BOB', 'ACTIVA', 0.00000000),
('BCO-INTERES-BOB', 'BOB', 'ACTIVA', 0.00000000);