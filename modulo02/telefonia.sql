...
$$
    ...
    BEGIN
        ...
        WITH planes_insertados AS (
            INSERT INTO planes_servicio (nombre_plan, tipo_plan, costo_mensual, limite_datos_gb,
                                         limite_minutos_voz) VALUES ('Plan Básico 4G', 'PREPAGO', 50.00, 10, 500),
                                                                    ('Plan Avanzado 5G', 'POSTPAGO', 150.00, 50, 2000),
                                                                    ('Plan Ilimitado PRO', 'POSTPAGO', 250.00, 999,
                                                                     9999)
                RETURNING id_plan)
        SELECT array_agg(id_plan)
        INTO arr_planes
        FROM planes_insertados;

        -- B. Insertar 3 Paquetes y capturar sus UUIDs
        WITH paquetes_insertados AS (
            INSERT INTO catalogo_paquetes (nombre_paquete, costo, volumen_datos_mb, minutos_voz, vigencia_dias) VALUES ('Bolsita WhatsApp 1 Día',
                                                                                                                        2.00,
                                                                                                                        200,
                                                                                                                        0,
                                                                                                                        1),
                                                                                                                       ('Combo Noche 1GB + 30min',
                                                                                                                        10.00,
                                                                                                                        1024,
                                                                                                                        30,
                                                                                                                        2),
        ...
        SELECT array_agg(id_paquete)
        INTO arr_paquetes
        FROM paquetes_insertados;
        ...
        WITH abonados_insertados AS (
            INSERT INTO abonados (id_plan, numero_documento, nombres, apellidos, numero_telefono, fecha_registro)
                SELECT arr_planes[trunc(random() * 3 + 1)::INT],
        ...
        SELECT array_agg(id_abonado)
        INTO arr_abonados
        FROM abonados_insertados;
        ...
        SELECT unnest(arr_abonados), -- Desenvolvemos el arreglo de UUIDs de abonados
        ...
        SELECT arr_abonados[trunc(random() * 5000 + 1)::INT],
        ...
        SELECT a.id_abonado,
               ...
               'FAC-' || TO_CHAR(NOW() - (meses.m * INTERVAL '1 month'), 'YYYYMM') || '-' ||
               SUBSTRING(a.numero_telefono FROM 5),
